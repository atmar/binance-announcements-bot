...
export default class PancakeswapProcessor {
  ...
  async buy(bnbToken: Token, buyToken: Token) {
    ...
    if (!AppConfig.config.testMode) {
      return await this.buyTokenOnCakeswap(Calculate.includeDecimals(this.BNB_TO_SELL, bnbToken.decimals), tokensToReceiveTotal, buyToken.address, bnbToken.address);
    }
  }

  async sell(bnbToken: Token, sellToken: Token) {
    const tokenBalanceAmount = await this.getBalanceOfToken(sellToken.address);

    const bnbToReceiveTotal = await this.cakeswap.getAmountsOut(tokenBalanceAmount, bnbToken.address, sellToken.address);
    const bnbToReceive = Calculate.excludeDecimals(bnbToReceiveTotal.toString(), bnbToken.decimals);
    
    //TODO: save the transaction in json file
    if (!AppConfig.config.testMode) {
      await this.approveToken(sellToken.address, tokenBalanceAmount);

      return await this.sellTokenOnCakeswap(bnbToReceiveTotal, tokenBalanceAmount, sellToken.address, bnbToken.address);
    }
  }
